...
function TransactionTable({ transactions, addTransaction, fetchTransactions }) {
  ...
  let filterTransactions = transactions.filter((item) => {
    // Check if item.name, search, item.type, and typeFilter have valid values before using includes()
    const itemName = item.name ? item.name.toLowerCase() : '';
    const searchInput = search ? search.toLowerCase() : '';
    const itemType = item.type || '';
    const filterType = typeFilter || '';
  
    return itemName.includes(searchInput) && itemType.includes(filterType);
  });
